//+------------------------------------------------------------------+
//| Infinity Hedge Trail EA                |
//+------------------------------------------------------------------+

// Input parameters
input double ATRMultiplier = 0.5;
input double TrailingATRMultiplier = 0.15;
input int ATRPeriod = 14;
input ENUM_TIMEFRAMES ATRTimeframe = PERIOD_CURRENT;
input ENUM_TIMEFRAMES TrailingATRTimeframe = PERIOD_CURRENT;
...
bool SendOrder(int orderType, double price, string comment)
{
   ...
   while (retry > 0)
   {
      int ticket = OrderSend(Symbol(), orderType, LotSize, price, 3, 0, 0, comment, MagicNumber, 0, clrGreen);
...
//+------------------------------------------------------------------+
void UpdateTrailingStop(int orderType, int ticket, double openPrice)
{
   double trailingStopDistance = atrValue * TrailingATRMultiplier;
   double newStopLoss = 0;

   if (orderType == OP_BUY)
   {
      newStopLoss = NormalizeDouble(Bid - trailingStopDistance, Digits);
      if (OrderStopLoss() < newStopLoss)
      {
         if (!OrderModify(ticket, openPrice, newStopLoss, OrderTakeProfit(), 0, clrGreen))
            Print("Error modifying Buy StopLoss: ", GetLastError());
      }
   }
   else if (orderType == OP_SELL)
   {
      newStopLoss = NormalizeDouble(Ask + trailingStopDistance, Digits);
      if (OrderStopLoss() > newStopLoss || OrderStopLoss() == 0)
      {
         if (!OrderModify(ticket, openPrice, newStopLoss, OrderTakeProfit(), 0, clrRed))
            Print("Error modifying Sell StopLoss: ", GetLastError());
      }
   }
}
   ...
   for (int i = OrdersTotal() - 1; i >= 0; i--)
   {
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderMagicNumber() == MagicNumber)
      {
         if (OrderType() == OP_BUY || OrderType() == OP_SELL)
         {
            ...
            UpdateTrailingStop(OrderType(), OrderTicket(), OrderOpenPrice());
